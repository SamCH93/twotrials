...
plot(museq, pfun1, type = "l", lwd = 1.5, xlab = "", ylim = c(0, 1),
     ylab = bquote(italic(P) * "-value"), xaxt = "n", yaxt = "n")
...
plot(museq, 2*pmin(pfun1, 1 - pfun1), type = "l", lwd = 1.5, xlab = "",
     ylim = c(0, 1),
     ylab = bquote("2min{" * italic(P) * "-value," * 1 - italic(P) * "-value}"),
     xaxt = "n", yaxt = "n")
mtext(side = 1, text = bquote(mu), line = 1.5)
axis(side = 2, at = pbks)
...
## ----"hypothetical-examples", fig.height = 5.5--------------------------------
...
resList <- lapply(X = seq(1, nrow(settingsgrid)), FUN = function(i) {
    ...
    list("p" = data.frame(plotres, settingsgrid[i,]),
         "summaries" = data.frame(rbind(isummaries, summaries), settingsgrid[i,]))
})
...
## ----"RESPIRE-analysis", fig.height = 5.5-------------------------------------
...
RESPIREtab <- RESPIREres$summariesDF |>
    mutate(ciwidth = upper - lower,
           across(c(lower, est, upper, ciwidth), ~ paste0("$",
                                                          sprintf("%.2f", .x), "$")),
           p0 =  paste0("$", sprintf("%.5f", p0), "$")) |>
    select(labels, type, lower, est, upper, ciwidth, p0)
kable(RESPIREtab |> select(-labels),
      format = "latex", booktabs = TRUE, align = "lrrrcc",
      col.names = c("",
                    "\\textbf{Lower CI}",
...
kable(ORBITtab |> select(-labels),
      ...
      col.names = c("",
                    ...
                    "\\textbf{\\textit{P}-value (one-sided)}"),
      bold = TRUE, escape = FALSE) |>
    pack_rows("\\textit{Primary endpoint (log hazard ratio)}", 1, 8, escape = FALSE,
              bold = FALSE) |>
    pack_rows("\\textit{Secondary endpoint (log rate ratio)}", 9, 16, escape = FALSE,
              bold = FALSE) |>
    row_spec(c(2, 10), hline_after = TRUE) |>
    row_spec(seq(1, nrow(ORBITtab), by = 2), background = "#F0F0F0")


## ----"morethan2", fig.height = 3.25-------------------------------------------
## parameters
...
## ----"R-package-illustration", echo = TRUE, fig.height = 4--------------------
...
## ----"fisher-pearson-edgington-approximation", fig.height = 4-----------------
...
se1 <- 0.05
se2 <- 0.07
p1 <- 1 - pnorm((t1 - museq)/se1)
p2 <- 1 - pnorm((t2 - museq)/se2)
