...
Following this rule, we can see that Fisher's method and meta-analysis lead to
success while the remaining methods do not. In sum, this example illustrates how
combined \textit{p}-value functions can be applied to more than two trials,
...
Data from the RESPIRE trials were extracted from Table~3 in De Soyza et al.
\citep{DeSoyza2018} and Table~3 in Aksamit et al. \citep{Aksamit2018}. Data from
the ORBIT trials were extracted from page~219 in Haworth et al.
\citep{Haworth2019}. Code and data to reproduce our analyses are openly
available at \url{https://github.com/SamCH93/twotrials}. A snapshot of the
...
using their equations (9) and (11), it can be shown that the expectations of $Y$
and $X$ are given by
\begin{align*}
  E(Y) =&
...
\bibliographystyle{wileyNJD-AMA} %{unsrtnat}
