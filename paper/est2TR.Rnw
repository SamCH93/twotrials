...
understood as a combined \textit{p}-value function based on the squared maximum of two
\textit{p}-values \citep{Held2024} which is a special case of Wilkinson's combination
method \citep{Wilkinson1951}, while meta-analysis corresponds to the combined
\textit{p}-value function based on Stouffer's \textit{p}-value combination method
\citep{Stouffer1949} with suitable weights. Both can be used to obtain combined
\textit{p}-values for the null hypothesis of no effect, CIs, and point estimates. These
quantities are compatible in the sense that the (two-sided) \textit{p}-value for a null
...
combined \textit{p}-value function pespective suggests considering alternative
\textit{p}-value combination methods, for example, Edgington's method based on the sum
of \textit{p}-values \citep{Edgington1972} or Fisher's method based on the product of
\textit{p}-values \citep{Fisher1934}.
...
combination method (after transforming \textit{p}-values to $z$-values) using inverse
...
test statistics underlying the \textit{p}-values~\eqref{eq:fisher}
...
Fisher's and Pearson's methods are related in the sense that the \textit{p}-value
function based on Fisher's method and the alternative hypothesis ``greater'' is
the same as one minus the \textit{p}-value function based on Pearson's method and the
...
\begin{table}[!htb]
  \centering
  \caption{Point estimates, 95\% CIs, and \textit{p}-values for
...
\begin{table}[!htb]
  \centering
  \caption{Point estimates, 95\% CIs, and \textit{p}-values for the ORBIT trials
    \citep{Haworth2019}.}
...
\textit{p}-value functions allow us to obtain point estimates, CIs, and \textit{p}-values that
...
Figure~\ref{fig:respireall} shows \textit{p}-value functions that combine all four
...
\textit{p}-value functions can be applied to more than two trials, while allowing to
...
Our findings also clarify how different \textit{p}-value combination methods implicitly
...
decisions based on a one-sided combined \textit{p}-value thresholded at $\alpha$ with
...
the underlying \textit{p}-value function, as in
...
combination of stage-wise \textit{p}-values \citep{Bauer1994, Lehmacher1999,
  Bauer1999}. Combined \textit{p}-value functions can be generalized to more than two
...
\citep{Borenstein2010}, and combining \textit{p}-value functions that are based on the
exact distribution of the data rather than normality \citep[e.g., the \textit{p}-value
...
Investigating whether other \textit{p}-value combination methods have similar Bayesian
