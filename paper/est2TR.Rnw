...
<< "check-closed-form", eval = FALSE >>=
par(mfrow = c(1, 2))
t1 <- 0.5
t2 <- t1
se1 <- 0.1
se2 <- se1
museq <- seq(0.3, 0.7, 0.01)
aseq <- seq(0.01, 0.99, 0.01)
alt <- "greater"
pfis <- pFisher(museq, t1, t2, se1, se2, alt)
pfis1 <- 1 - pchisq(q = -4*log(1 - pnorm((t1 - museq)/se1)), df = 4)
ppear <- pPearson(museq, t1, t2, se1, se2, alt)
ppear1 <- pchisq(q = -4*log(pnorm((t1 - museq)/se1)), df = 4)
mufis <- muFisher(aseq, t1, t2, se1, se2, alt)
mufis1 <- t1 + se1*qnorm(exp(-qchisq(1 - aseq, df = 4)/4))
mupear <- muPearson(aseq, t1, t2, se1, se2, alt)
mupear1 <- t1 - se1*qnorm(exp(-qchisq(aseq, df = 4)/4))
matplot(museq, cbind(pfis, ppear), lty = 2, col = c(2, 4), type = "l",
        xlab = "mu", ylab = "p")
matlines(museq, cbind(pfis1, ppear1), lty = 3, col = c(2, 4), type = "l")
legend("topleft", legend = c("Fisher", "Pearson"), col = c(2, 4), lty = 1)
matplot(aseq, cbind(mufis, mupear), lty = 2, col = c(2, 4), type = "l",
        xlab = "a", ylab = "mu")
matlines(aseq, cbind(mufis1, mupear1), lty = 3, col = c(2, 4), type = "l")
