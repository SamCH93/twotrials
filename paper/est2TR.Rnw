...
\citep{Shun2005, Zhan2022}, a CI based on a combined $p$-value function has
...
$p$-value functions $g$, and how they behave when the true effects are not the
...
other types of combined $p$-value functions. As these investigations are
